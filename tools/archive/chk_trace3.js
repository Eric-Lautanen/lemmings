...
for (let t = 0; t < 160; t++) {
  ...
  if (t < 55) continue;
  if (lem0.dead || lem0.rescued) { console.log('t' + t, 'end', lem0.dead ? 'dead' : 'rescued'); break; }
  ...
  } else { r = 1; while (!Lc.solid[(Math.round(lem0.y) + 1 + r) * Lc.W + fx] && r < 4) r++; }
  console.log('t' + t, lem0.state, 'x', Math.round(lem0.x), 'y', Math.round(lem0.y), 'dir', lem0.dir, 'front', fx, 'dy', r, 'climber', lem0.climber);
}